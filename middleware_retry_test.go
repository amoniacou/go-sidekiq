...
func MiddlewareRetrySpec(c gospec.Context) {
	...
	c.Specify("handles recurring failed message", func() { // nolint: dupl
	...
	c.Specify("handles recurring failed message with customized max", func() { // nolint: dupl
	...
	c.Specify("use retry_options when provided - min_delay", func() {
		...
		msg := ""
		nextAt := -1.0
		_, err := redis.Scan(values, &msg, &nextAt)
